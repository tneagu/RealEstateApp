...
class ListingDetailsViewModelTest {
    ...
    // Test Data
    private val sampleListingDetail = ListingDetail(
        id = testListingId,
        bedrooms = 3,
        city = "Paris",
        area = 120.5,
    ...
    @Test
    fun `ViewModel retrieves listingId from SavedStateHandle`() = runTest {
    ...
    @Test
    fun `handleIntent LoadDetails emits Loading then Success state`() = runTest {
        // Given
    ...
    @Test
    fun `handleIntent LoadDetails uses correct listingId from intent`() = runTest {
        // Given
    ...
    @Test
    fun `handleIntent LoadDetails emits Loading then Error on NetworkUnavailable`() = runTest {
        // Given
    ...
    @Test
    fun `handleIntent LoadDetails emits Loading then Error on ServerError`() = runTest {
        // Given
    ...
    @Test
    fun `handleIntent LoadDetails emits Loading then Error on UnknownError`() = runTest {
        // Given
    ...
    @Test
    fun `handleIntent Retry successfully loads details after previous failure`() = runTest {
        // Given
        val errorResult = DataResult.Failure(DomainError.ServerError())
        val successResult = DataResult.Success(sampleListingDetail)
        coEvery { getListingDetailsUseCase(testListingId) } returnsMany listOf(
            errorResult,
            successResult
        )
    ...
    @Test
    fun `handleIntent NavigateBack emits NavigateBack effect`() = runTest {
        // When
